    ...
    if form.validate_on_submit():
        ...
        if form.image.data:
            ...
            file_path = os.path.join(
                current_app.config['BLOG_POST_IMAGES_PATH'], file_name
            )
            Image.open(f).save(file_path)
            # create sizes
            _image_resize(current_app.config['BLOG_POST_IMAGES_PATH'], image_id, 600, 'lg')
            _image_resize(current_app.config['BLOG_POST_IMAGES_PATH'], image_id, 300, 'sm')
        
            post = Post(body=form.post.data, author=current_user,
                language=language,image=image_id)
        else:
