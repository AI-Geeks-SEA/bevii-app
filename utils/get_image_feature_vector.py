...
def get_image_feature_vectors():
 BLOG_POST_IMAGES_PATH=os.environ.get('BLOG_POST_IMAGES_PATH') 
 # Definition of module with using tfhub.dev
 module_handle = "https://tfhub.dev/google/imagenet/mobilenet_v2_140_224/feature_vector/4"
 # Loads the module
 module = hub.load(module_handle)
 # Loops through all images in a local folder
 for filename in glob.glob(BLOG_POST_IMAGES_PATH+'/*.sm.png'):
...
# Remove single-dimensional entries from the 'features' array  
  ...
  out_path = os.path.join(BLOG_POST_IMAGES_PATH,
            outfile_name)
